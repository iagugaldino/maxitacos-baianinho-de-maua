document.addEventListener("DOMContentLoaded", function() {
    ...
    function initCarousel() {
        ...
        const images = [
            'images/mbSmrm0xYkHw.webp',
            'images/8KGhV5pd7hh3.webp', 
            'images/IJkIp4vZpFIm.webp',
            'images/htvXq8w0gTDi.webp',
            'images/i13QM2WQJwch.webp',
            'images/Oyy4OFXfaSlO.webp',
            'images/5k3JVI3ZyTVE.webp'
        ];
    ...
    colorRadios.forEach((radio) => {
        function handleColorChange() {
            ...
            const variationOption = radio.closest('.variation-option');
        ...
        // Adicionar listeners para change e click
        radio.addEventListener('change', handleColorChange);
        radio.addEventListener('click', handleColorChange);
        
        // Também adicionar listener na label
        const label = radio.closest('.variation-option');
        if (label) {
            label.addEventListener('click', function(e) {
                // Se clicou na label, selecionar o radio
                radio.checked = true;
                handleColorChange();
            });
        }
    ...
    // Função de teste para verificar links
    window.testColorLinks = function() {
        console.log('=== TESTING COLOR LINKS ===');
        const allRadios = document.querySelectorAll('input[name="color"]');
        allRadios.forEach((radio, index) => {
            const variationOption = radio.closest('.variation-option');
            const colorName = variationOption.getAttribute('data-color');
            const link = variationOption.getAttribute('data-link');
            console.log(`Radio ${index}: ${colorName} -> ${link}`);
        });
        
        const buyButton = document.getElementById('btn-comprar-original-unico');
        console.log('Current buy button href:', buyButton ? buyButton.href : 'not found');
    };
    
    // Expor função para teste
    console.log('Test function available: window.testColorLinks()');
